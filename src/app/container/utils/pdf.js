...
export const pdfDataL = function (doc, details) {
    let insinfox = localStorage.getItem('companyInfo');

    let insinfo = JSON.parse(insinfox);
    var instituteName = insinfo?.companyName
    var instituteAddress = insinfo?.address
    var imgData = 'data:image/gif;base64,R0lGODlhAQABAIAAAP7//wAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==';
    // return doc.addImage(imgData, "JPEG", 15, 12, 20, 20),
    return doc.setFontSize(15), doc.writeText(20, 20, instituteName, { align: "left" }),
        doc.setFontSize(9), doc.writeText(20, 26, instituteAddress, { align: "left" }),
